module LevelBuilder exposing (blok, box, cartesianProduct, corner, cross, deadEnd, diode, frame, hCross, hFlip, hFlipTransformation, hStick, lBlok, oneWay, rotate, rotateTransformation, transformBlok, twoWay, union, vFlip, vFlipTransformation, vStick)
...
hStick mat ( x, y ) size =
    List.map2 Tuple.pair (List.range x (x + size - 1)) (List.repeat size y)
        |> blok mat


vStick : Material -> Loc -> Int -> Blok
vStick mat ( x, y ) size =
    List.map2 Tuple.pair (List.repeat size x) (List.range y (y + size - 1))
        |> blok mat
...
transformBlok transf blk =
    let
        origin =
            List.head (Set.toList blk.structure)
    in
    case origin of
        Just o ->
            Set.map (transf o) blk.structure |> Set.toList |> blok blk.material
