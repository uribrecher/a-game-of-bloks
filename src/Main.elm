module Main exposing (Model, andThenPred, init, isFail, main, subscriptions, update, updateGravity, updateModel, updateModelCleared, updateModelNormal, updateReplay, view, viewBackGround, viewReplayButton)
...
subscriptions model =
    Sub.batch
        [ Browser.Events.onKeyDown (Json.Decode.map KeyDown BaseTypes.keyDecoder)
        , Browser.Events.onKeyUp (Json.Decode.map KeyUp BaseTypes.keyDecoder)
        , Time.every 50 Tick -- consider replacing with onAnimationFrame
        ]
...
viewBackGround =
    rect
        ...
        , x "0"
        , y "0"
        , width (String.fromInt (rectSize * boundsX))
        , height (String.fromInt (rectSize * boundsY))
        ]
        []
...
view model =
    Html.div []
        [ Svg.svg [ viewBox "0 0 100 100", width "700px" ]
            ...
            , Svg.text_ [ x "10", y "5", fontSize "7", fill "#ffffff" ]
                [ Svg.text
                    (if model.fail then
                        "Oh no!"
